...
app.post("/create-checkout-session", async (req, res) => {
  try {
    console.log("Creating checkout session with body:", JSON.stringify(req.body))
    ...
    const customerPostcode = req.body.customerPostcode
  ...
  try {
    ...
    try {
      if (session.metadata && session.metadata.cartItems) {
        ...
        // Otherwise build from line items
